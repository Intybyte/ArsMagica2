...
public class ParticleManagerClient extends ParticleManagerServer{
	...
	public void spawnAuraParticles(EntityLivingBase ent){
		...
		ConfigBeta configBeta = AMConfig.getInstance().getBeta();

		if (Minecraft.getMinecraft().thePlayer == ent){
			particleIndex = configBeta.getAuraType();
			particleBehaviour = configBeta.getAuraBehaviour();
			particleScale = configBeta.getAuraScale() / 10;
			particleAlpha = configBeta.getAuraAlpha();
			particleDefaultColor = configBeta.isAuraDefaultColor();
			particleRandomColor = configBeta.isAuraRandomColor();
			particleColor = configBeta.getAuraColor();
			particleQuantity = configBeta.getAuraQuantity();
			particleSpeed = configBeta.getAuraSpeed() / 10;
		}else{
	...
	public void spawnBuffParticles(EntityLivingBase entityliving){
		...
		handlePotion(entityliving, world);
		if (Minecraft.getMinecraft().gameSettings.thirdPersonView == 0) {
		}
	}

	private void handlePotion(EntityLivingBase entityliving, World world) {
