...
public class Appropriation implements ISpellComponent{
	...
	public boolean applyEffectBlock(ItemStack stack, World world, int blockx, int blocky, int blockz, int blockFace, double impactX, double impactY, double impactZ, EntityLivingBase caster){
		...
		for (String s : AMConfig.getInstance().getGeneral().getAppropriationBlockBlacklist()){
			if (Objects.equals(block.getUnlocalizedName(), s)){
				return false;
			}
		}
	...
	public boolean applyEffectEntity(ItemStack stack, World world, EntityLivingBase caster, Entity target){
		...
		for (Class clazz : AMConfig.getInstance().getGeneral().getAppropriationMobBlacklist())
	...
	public void spawnParticles(World world, double x, double y, double z, EntityLivingBase caster, Entity target, Random rand, int colorModifier){
		for (int i = 0; i < 5 + 5 * GfxUtil.get(); ++i){
